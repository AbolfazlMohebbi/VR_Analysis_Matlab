...
for i = Trials_Data_All.VelCase
    ...
    for j = Trials_Data_All.AmpCase
        ...
        VAF_TQ_mean_vec = [VAF_TQ_mean_vec, mean(VAF_TQ_allSubj)];
        VAF_Body_mean_vec = [VAF_Body_mean_vec, mean(VAF_Body_allSubj)];
        ...
        figure(FigNo_VAF);
        err = std(VAF_Body_mean_vec)*ones(size(VAF_Body_mean_vec));
        errorbar(VAF_Body_mean_vec, err); hold on
        ...
        figure(FigNo_IRF+1);
        err = std(VAF_TQ_mean_vec)*ones(size(VAF_TQ_mean_vec));
        errorbar(VAF_TQ_mean_vec,err); hold on
        set(gca, 'XTickLabel', TrialTicks)
        xtickangle(-45)
        set(gcf,'Units','Normalized','OuterPosition',[0.1 0.1 0.4 0.8]);
