...
Subject_data = textscan(fid, '%d %s %d %d %d %d %d %d %d %d %s %s', 'Delimiter', ',', 'HeaderLines', 1);
fclose(fid);
...
elseif ((stackTrials == 2) && (Averaging == 0))  %No stacking
    ...
    % VR
    VRnldat = nldat;
    set(VRnldat,'dataSet', vr_realizations' * (pi/180) , 'domainIncr',1/SR);
    
    % EMGs Left
    EMG_LSol = nldat; 
    set(EMG_LSol,'dataSet', LSol' ,'domainIncr', 1/SR);
    
    EMG_LMG = nldat;
    set(EMG_LMG,'dataSet', LMG' ,'domainIncr', 1/SR);
    
    EMG_LLG = nldat;
    set(EMG_LLG,'dataSet', LLG' ,'domainIncr', 1/SR);
    
    EMG_LTA = nldat;   
    set(EMG_LTA,'dataSet', LTA' ,'domainIncr', 1/SR);    
...
xlabel('Time (s)')
yyaxis right
ax(1)= plot(t,VRnldat.dataSet);hold on
ylabel('VR input angle (Deg)');
yyaxis left
ax(2)= plot(t,EMG_LSol.dataSet);
ax(3) = plot(t, EMG_LMG.dataSet);
ax(4) = plot(t, EMG_LLG.dataSet);
ax(5) = plot(t, EMG_LTA.dataSet);
ylabel('Muscle Activations (V)');
legend([ax(2) ax(3) ax(4) ax(5)], 'Soleus Muscle EMG (V)', 'Medial Gastroc Muscle EMG (V)', 'Lateral Gastroc Muscle EMG (V)', 'Tibialis Anterior Muscle EMG (V)');
